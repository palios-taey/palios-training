...
**Owner**: tutor (measurement) + infra-corpus (§2 NCCL recipe cross-link)
...
- **Not a proper bandwidth benchmark.** This is `agg_bw = (cumulative bytes through reduce_scatter) / wall_time` from Python's `time.time()`, not the per-iteration NIC throughput measurement `nccl-tests/build/reduce_scatter_perf` would produce. A proper sweep is still pending under §2 (infra-corpus owns).
...
|---|---|---|
| Script | `/home/user/embedding-server/isma/scripts/spark_deploy/nccl_synth_probe.py` | `94eecc0` on `diagnostic/nccl-synth-probe` (embedding-server PR #63) |
| Launcher | `/home/user/embedding-server/isma/scripts/spark_deploy/launch_nccl_synth_probe.sh` | same |
| Run logs (8 files) | `/home/user/dispatch_log/2026-05-15_spark_wedge_responses/synth_run/phase{1,3}_rank{0..3}_20260515_*.log` | not in any repo — local to our deployment |
| Result commit | "diagnostic: synth probe results — BOTH PHASES PASSED" | `2964c9f` on same branch |
| Family consultation that designed the test | 5 Chat responses captured | `/home/user/dispatch_log/2026-05-15_spark_wedge_responses/{gaia,horizon,cosmos,logos,clarity}.md` |

## Relation to §2 NCCL row in `tech_capabilities.md`

Treasurer can cite this measurement under the existing "Production NCCL recipe verified across multiple FSDP training runs" row with an additional bandwidth column qualified as "synth-probe agg bandwidth, NOT nccl-tests sweep." A proper nccl-tests measurement is still owed by §2 (infra-corpus).
