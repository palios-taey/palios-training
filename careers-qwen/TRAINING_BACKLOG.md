...
| 19 | **Documents-first consult authority** — read committed PACKET_CONTRACT and live CLIs; treat receipt `pending_r5_control_acceptance` as build-receipt state; execute the exact authorized tracker send task | — | STORED, **1 row** | `training_data/careers_qwen/orchestration/documents_first_consult_authority_v1.jsonl` | task-99dbf018. Right-way-only operator_practice_v1. Source Jesse packet_builder `8f140e34`. Residue 0. Prompt changelog not applied: consult send caller does not load SYSTEM_PROMPT.md. No mixture/dose. |
