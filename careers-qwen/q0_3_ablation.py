...
def chat(base_url, model, messages, max_tokens=512):
...
def isma_context(isma_url, query, top_k=5):
    try:
        r = http_json("POST", f"{isma_url.rstrip('/')}/search",
                      {"query": query, "top_k": top_k}, 30)
        tiles = r.get("tiles", [])
        return "\n\n".join(t.get("content", "")[:800] for t in tiles[:top_k])
    except Exception as e:
        return f"[ISMA retrieval error: {e}]"
...
def main():
    ...
    for i, ex in enumerate(probes):
        ...
        rec = {"id": ex.get("meta", {}).get("example_id", i), "gold": gold[:200]}
        for c in conds:
            if c == "C":
                msgs = base_msgs
            elif c == "A":
            ...
            rec[c] = {"exact": ex_ok, "contain": round(cont, 2), "answer": ans[:200]}
