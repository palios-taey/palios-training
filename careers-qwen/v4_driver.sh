...
# Does NOT auto-kill — tutor watches ${SPARK_HOME}/module1_v4.log for the V4 criteria, then decides.
set -uo pipefail
...
ssh -o ConnectTimeout=8 spark@"$MASTER" "cd ${SPARK_HOME} && tmux kill-session -t module1 2>/dev/null; tmux new-session -d -s module1 \"SAVE_EVERY=30 SESSION_SECONDS=3600 bash launch_module1_fsdp.sh 0 > ${SPARK_HOME}/module1_v4.log 2>&1\"" 2>/dev/null
...
for n in ${SPARK_NODE1} ${SPARK_NODE2} ${SPARK_NODE3}; do
  ssh -o ConnectTimeout=8 spark@"$n" "cd ${SPARK_HOME} && tmux kill-session -t module1 2>/dev/null; tmux new-session -d -s module1 \"SAVE_EVERY=30 SESSION_SECONDS=3600 bash launch_module1_fsdp.sh $r > ${SPARK_HOME}/module1_v4.log 2>&1\"" 2>/dev/null
  say "rank $r launched in tmux on .${n##*.}"; r=$((r+1))
done
say "V4 LAUNCHED on all 4. Watch: ssh spark@$MASTER 'tail -f $HOME/module1_v4.log'"
