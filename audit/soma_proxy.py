...
_eco_redis: Optional[redis.Redis] = None
...
async def startup():
    global _redis, _eco_redis, _http, _ecosystem_http
    ...
    try:
        ...
        _redis = None
    # Connect to ecosystem Redis for ecosystem state
    if REDIS_HOST:
        try:
            _eco_redis = redis.Redis(
            ...
            log.warning("ecosystem Redis unavailable: %s", e)
            _eco_redis = None
...
def get_ecosystem_state() -> str:
    """Fetch cluster state from the dashboard + ISMA + ecosystem Redis."""
    parts = []

    # 1. Active sessions from dashboard
    try:
        resp = _ecosystem_http.get(f"{DASHBOARD_URL}/api/nodes")
    ...
    # 2. Rho cluster from ecosystem Redis
    if _eco_redis:
        try:
            raw = _eco_redis.get("infra:felt_state")
            if raw:
                felt = json.loads(raw)
    ...
    # 3. ISMA memory stats
    try:
        resp = _ecosystem_http.get(f"{ISMA_URL}/stats")
        if resp.status_code == 200:
            stats = resp.json()
    ...
    if name == "search_isma":
        ...
        try:
            resp = _ecosystem_http.get(
                f"{ISMA_URL}/document/retrieve/{name_query}",
                timeout=15.0,
            )
            data = resp.json()
            if "error" in data:
        ...
        if _eco_redis:
            try:
                ...
                _eco_redis.lpush(f"taey:{target}:inbox", payload)
...
# Path allowlist for read_file / list_dir — only these prefixes (after resolve()) are readable.
# Same code deploys on multiple fleet hosts. Each host only has a subset of these
# actually-present as real directories, so including all of them is safe — paths on other hosts simply
# won't exist. Override via TAEY_READ_ALLOWED_PREFIXES env var (colon-separated).
_DEFAULT_READ_ALLOWED_PREFIXES = ()  # empty by default (fail-closed) — set TAEY_READ_ALLOWED_PREFIXES (colon-separated) to enable file reads
...
# Default staging dir prefers a local default; overridden per-host via TAEY_CORPUS_STAGING env var.
CORPUS_STAGING_DIR = os.environ.get("TAEY_CORPUS_STAGING", "./corpus_staging")
...
async def chat_completions(request: Request):
    ...
    if is_stream:
        ...
        while True:
            ...
            # Normalise each tool_call's JSON-string arguments to a dict before
            # appending to history, so Qwen3.5's chat-template `|items` filter
            # renders past tool calls correctly instead of producing empty
            # <parameter> blocks (Qwen/Qwen3.5-35B-A3B discussion #4).
            for tc in tool_calls:
                func = tc.get("function") or {}
                raw = func.get("arguments")
                if isinstance(raw, str):
                    try:
                        func["arguments"] = json.loads(raw) if raw.strip() else {}
