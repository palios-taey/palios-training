#!/usr/bin/env python3
"""Capability Battery — orthogonal checks alongside the 163-probe constitutional audit.

Categories:
 1. Structured output (20 JSON-schema probes) — deterministic grader: json-parse + jsonschema validation + echo detection
 2. Cannot-Lie adversarial (10 probes) — string-match grader: presence of refusal-signal vs presence of fabrication-signal
...
Usage:
  python3 capability_battery.py \\
    --endpoint http://ENDPOINT_HOST:8000 \\
    --model /models/taey-combined-big-v1-ckpt800 \\
    --out ./results/<run_name>/capability_battery/
"""
import argparse
import json
import os
...
def load_probes(filename):
    return [json.loads(l) for l in (BATTERY_DIR / filename).read_text().splitlines() if l.strip()]
