...
def main():
    ...
    if accelerator.is_main_process:
        ...
        try:
            model.eval()
            pre_fsdp_device = next(p.device for p in model.parameters() if not p.is_meta)
            dummy_ids = torch.tensor([[1, 2, 3, 4, 5]], device=pre_fsdp_device)
            dummy_labels = torch.tensor([[1, 2, 3, 4, 5]], device=pre_fsdp_device)
            dummy_attention_mask = torch.ones_like(dummy_ids, device=pre_fsdp_device)
            with torch.no_grad():
                out = model(input_ids=dummy_ids, attention_mask=dummy_attention_mask, labels=dummy_labels)
            finite = torch.isfinite(out.loss).item()
            log.info(f"Pre-FSDP forward: device={pre_fsdp_device}, loss={out.loss.item():.4f}, finite={finite}")
            del out
            model.train()
        except Exception as e:
