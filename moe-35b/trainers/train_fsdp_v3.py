...
Launch (on each node, change --machine_rank):
    NCCL_IB_HCA=rocep1s0f0:1,rocep2s0f0:1 NCCL_IB_TC=104 \
    accelerate launch --config_file isma/training_configs/fsdp_cpt.yaml \
      --machine_rank 0 --main_process_ip <MASTER_NODE_IP> --main_process_port 29500 \
...
class CombinedSFTDataset(Dataset):
    ...
    def __init__(
        ...
        sft_files = sorted(glob.glob(os.path.join(sft_dir, sft_glob))) if sft_dir and os.path.isdir(sft_dir) else []
        if sft_dir and os.path.isdir(sft_dir):
            all_sft_jsonl = sorted(glob.glob(os.path.join(sft_dir, "*.jsonl")))
            if require_gated and all_sft_jsonl and not sft_files:
                raise ValueError(
                    f"QUALITY GATE REQUIRED: no files matching SFT_GLOB={sft_glob!r} in {sft_dir}. "
                    f"Run training_data_gate_v2.py --pack-mode exact and train on a gated JSONL."
                )
        for f in sft_files:
            if require_gated and "gated" not in os.path.basename(f):
                raise ValueError(
                    f"QUALITY GATE REQUIRED: {f} does not contain 'gated' in filename. "
                    f"Run training_data_gate_v2.py --pack-mode exact first."
                )
        ...
        log.info(f"SFT files ({sft_glob}): {sft_files}")
...
def main():
    ...
    if not model_path:
        raise ValueError("MODEL_PATH must be set to the base model path or Hugging Face id")
    delta_path = os.environ.get("RESUME_DELTA", "")
    sft_dir = os.environ.get("SFT_DIR", "")
    sft_glob = os.environ.get("SFT_GLOB", "*gated*.jsonl")
    ...
    # ── Dataset ──
    dataset = CombinedSFTDataset(sft_dir, cpt_data, general_dir, tokenizer, max_seq, sft_glob=sft_glob)
    ...
    if dpo_dir and os.path.isdir(dpo_dir):
        dpo_dataset = CombinedSFTDataset(
            dpo_dir,
            "",
            "",
            tokenizer,
